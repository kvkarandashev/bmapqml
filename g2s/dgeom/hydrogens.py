...
def lebedev_sphere_opt(bond, points_distances, origin):
    ...
    deltas = np.sum(
        [
            (np.linalg.norm(candidate_positions - (pos - origin), axis=1) - dist) ** 2
            for pos, dist in points_distances
        ],
        axis=0,
    )
...
def get_hydrogen_positions(positions, distances, n_hydrogens):
    ...
    pos_1 = positions[0]
    dist_1 = distances[0]
    pos_hydr_1 = lebedev_sphere_opt(
        bond=dist_1, origin=pos_1, points_distances=zip(positions[1:], distances[1:-1])
    )
    if n_hydrogens > 1:
        p_dist_pairs = (
            list(zip(positions[1:-1], distances[1:-2]))
            if len(positions) > 2
            else list(zip(positions[1], distances[1]))
        )
        p_dist_pairs.append((pos_hydr_1, h_h_distance))
        pos_hydr_2 = lebedev_sphere_opt(
            bond=dist_1, origin=pos_1, points_distances=p_dist_pairs
        )
    else:
        pos_hydr_2 = None
    if n_hydrogens == 3:
        p_dist_pairs = [
            ...
            (pos_hydr_1, h_h_distance),
            (pos_hydr_2, h_h_distance),
        ]
        pos_hydr_3 = lebedev_sphere_opt(
            bond=dist_1, origin=pos_1, points_distances=p_dist_pairs
        )
    else:
        pos_hydr_3 = None
...
def hydrogen_lebedev_reconstruction(
    heavy_atom_coords, predicted_h_distances, heavy_atom_hydrogen_mapping
):
    """
    ...
    for i in range(len(heavy_atom_hydrogen_mapping)):
        ...
        pos_hydr_1, pos_hydr_2, pos_hydr_3 = get_hydrogen_positions(
            pred_positions, pred_h_distances, n_hydrogens=n_hydrogens
        )
