...
print(output)
X_init = local_space_sampling.gen_soap(
    crds, chgs
)  # gen_fchl(crds, chgs) #gen_bob(crds, chgs)

dl, dh = 450, 500
respath = "."

min_func = local_space_sampling.sample_local_space_3d(
    X_init,
    chgs,
    verbose=False,
    epsilon=50,
    gamma=dl,
    sigma=dh,
    repfct=local_space_sampling.gen_soap,
)
