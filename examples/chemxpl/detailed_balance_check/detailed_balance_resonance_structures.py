...
for MC_step in range(num_MC_steps):
    ...
    cur_egc = rw.change_molecule(
        cur_egc, randomized_change_params=randomized_change_params
    )
...
for egc, distr in zip(histogram_labels, histogram):
    ...
    i = nhatoms_is[egc.num_heavy_atoms()]
    distrs[i].append(distr)

for nhatoms in nhatoms_is:
    i = nhatoms_is[nhatoms]
    d = np.array(distrs[i])
    print(
        "Mean and standard deviation for nhatoms", nhatoms, ":", np.mean(d), np.std(d)
    )
