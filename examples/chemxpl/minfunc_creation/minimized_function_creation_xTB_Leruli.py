...
def est_stddev_vals(quant_estimate, tp_array):
    ...
    for i, tp in enumerate(tp_array):
        print(i, tp)
        quant_vals_tps.append(quant_est_wtp(tp, quant_estimate))
...
est_kwargs = {"coord_calculation_type": "Leruli"}

quant_estimates = [
    quant_estimate_type(**est_kwargs) for quant_estimate_type in quant_estimate_types
]
...
for quant_sign, quant_estimate, quant_name in zip(
    ...
    print("Calculating quantities for:", quant_name)
...
dump2pkl(
    min_function,
    pkl_store_dir + "/minimized_function_xTB_Leruli_electrolyte.pkl",
)
dump2pkl(
    est_vals,
    pkl_store_dir + "/QM9_data_min_func_xTB_Leruli_electrolyte.pkl",
)
