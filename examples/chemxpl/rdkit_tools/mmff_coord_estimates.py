...
from molopt.chemxpl.utility import egc_with_coords, write_egc2xyz
...
for mol_id, (nuclear_charges, bond_orders) in enumerate(zip([nuclear_charges1, nuclear_charges2], [bond_orders1, bond_orders2])):

    cg=ChemGraph(nuclear_charges=nuclear_charges, bond_orders=bond_orders, hydrogen_autofill=True)
    egc1=ExtGraphCompound(chemgraph=cg)
    print(egc1)
    egc2=egc_with_coords(egc1)
