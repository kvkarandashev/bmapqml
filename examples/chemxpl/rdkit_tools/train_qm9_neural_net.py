...
class Net(nn.Module):
    def __init__(
        ...
        self.sequ_layer = torch.nn.Sequential(
            ...
            act_fct,
            torch.nn.Linear(6 * self.n_neuo, self.n_neuo),
            act_fct,
    ...
    def fit(self, xtrain, ytrain):
        ...
        batch_size = 30  # 512 # 128 # or whatever
...
if __name__ == "__main__":
    ...
    SMILES, y = qm9_data["canon_smiles"].values, np.float_(
        qm9_data[TARGET_PROPERTY].values
    )  # *conversion_coefficient["au_eV"]
    print(np.mean(y))
    ...
    model = Net(
        ...
        num_epochs=600,
        verbose=True,
        name="/store/common/jan/qm9_removed/ml/NN_{}_{}".format(
            max_train_size, TARGET_PROPERTY
        ),
    )
    ...
    for n in N:
        ...
        y_pred = sct.inverse_transform(
            np.array(model.predict(X_test.cuda()).detach().cpu())
        )
    ...
    dump2pkl(
        report,
        "/store/common/jan/qm9_removed/ml/NN_report_{}.pkl".format(TARGET_PROPERTY),
    )
