...
change_prob_dict=[add_heavy_atom_chain, remove_heavy_atom, replace_heavy_atom, change_bond_order, change_valence]
...
rw=RandomWalk(bias_coeff=bias_coeff, randomized_change_params=randomized_change_params, bound_enforcing_coeff=bound_enforcing_coeff,
                keep_histogram=True, num_replicas=1)
...
for attempt_counter in range(num_attempts):
    dist=SMILES_walking_distance(SMILES1, SMILES2, rw, max_num_MC_steps=max_num_MC_steps)
    print(dist)
