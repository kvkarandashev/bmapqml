...
class ExtGraphCompound(GraphCompound):
    def __init__(self, adjacency_matrix=None, nuclear_charges=None, chemgraph=None, coordinates=None, distances=None, elements=None, hydrogen_autofill=False, bond_orders=None):
        ...
        for atom_id1 in range(self.chemgraph.nhatoms()):
            for atom_id2 in range(self.chemgraph.nhatoms()):
                self.adjacency_matrix[atom_id1, atom_id2]=self.chemgraph.bond_order(atom_id1, atom_id2)
        self.additional_data={} # In case we want to attach more data to the same entry.
    ...
    def resort_atoms(self):
        ...
        for atom_id, atom_charge in enumerate(self.nuclear_charges):
            if atom_charge == 1:
                ...
                heavy_atom_id+=1
        self.adjacency_matrix = self.adjacency_matrix[self.resort_atom_idx][:, self.resort_atom_idx]
        self.nuclear_charges = self.nuclear_charges[self.resort_atom_idx]
        if self.distances is not None:
            self.distances = self.distances[self.resort_atom_idx][:, self.resort_atom_idx]
        if self.coordinates is not None:
...
#   A copy of what is done in G2S with an extra option to allow sorting in the other order (makes more sense for Coulomb matrices, I think).
    def sort_norm(self, descending=False, canonical_ordering_sort=False, norm_added_delta=None):
        ...
        if canonical_ordering_sort:
            ...
            if norm_added_delta is not None:
                if self.chemgraph.canonical_permutation is None:
                    self.chemgraph.init_canonical_permutation()
                row_norms-=norm_added_delta*np.array(self.chemgraph.canonical_permutation)
            if descending:
                row_norms*=-1
    ...
    def generate_res_av_coulomb_matrix(self, size=9, descending_sorting=True, sort_up_to_eff_coord_num=None, canonical_ordering_sort=False, use_bond_order_dists=False, norm_added_delta=1e-6):
        dist_mats=self.get_res_av_dist_mats(sort_up_to_eff_coord_num=sort_up_to_eff_coord_num, use_bond_order_dists=use_bond_order_dists)
            ...
            for i in range(self.num_heavy_atoms()):
                ...
                for j in range(i):
                    if math.isinf(dist_mats[dm_id][i,j]):
                        ...
                        dist_mats[dm_id][i,j]*=self.nuclear_charges[i]*self.nuclear_charges[j]
                    dist_mats[dm_id][j,i]=dist_mats[dm_id][i,j]
        self.representation=dist_mats[0]
        self.sort_norm(descending=descending_sorting, canonical_ordering_sort=canonical_ordering_sort, norm_added_delta=norm_added_delta)
    ...
    def generate_res_av_md_coulomb_matrix(self, size=9, descending_sorting=True, sort_up_to_eff_coord_num=None,
                            canonical_ordering_sort=True, use_bond_order_dists=False, norm_added_delta=1e-6):
        # 
        atom_multipliers=np.array([list(ha.comparison_iterator()) for ha in self.chemgraph.hatoms]).T
        dist_mats=self.get_res_av_dist_mats(sort_up_to_eff_coord_num=sort_up_to_eff_coord_num, use_bond_order_dists=use_bond_order_dists)
        self.representation=np.zeros((self.num_heavy_atoms(), self.num_heavy_atoms()), dtype=float)
        rep_components=[]
    ...
    def augment_representation(self, extra_mats, size, k=1, matrices_asymmetric=False):
        if len(extra_mats)!=0:
            ...
            for extra_mat in extra_mats[1:]:
                temp_mat[:self.num_heavy_atoms(), :self.num_heavy_atoms()]=extra_mat[self.sorting_idxs][:, self.sorting_idxs]
                if matrices_asymmetric:
                    self.representation=np.append(self.representation, np.reshape(temp_mat, (size**2,)))
                else:
                    self.representation=np.append(self.representation, temp_mat[np.triu_indices(size, k=k)])

    def generate_res_av_bond_length(self, size=9, sort_up_to_eff_coord_num=None, canonical_ordering_sort=False, norm_added_delta=1e-6):
        dist_mats=self.get_res_av_dist_mats(sort_up_to_eff_coord_num=sort_up_to_eff_coord_num)
        self.representation=dist_mats[0]
        self.sort_norm(canonical_ordering_sort=canonical_ordering_sort, norm_added_delta=norm_added_delta)
        self.zero_padding(size)
        self.representation = self.representation[np.triu_indices(size, k=1)]
        self.augment_representation(dist_mats[1:], size)

    def get_res_av_dist_mats(self, sort_up_to_eff_coord_num=None, use_bond_order_dists=False):
        edge_list=self.chemgraph.graph.get_edgelist()
        if use_bond_order_dists:
            bond_lengths=1.0/self.chemgraph.get_res_av_bond_orders(edge_list=edge_list)
        else:
        ...
        if sort_up_to_eff_coord_num is not None:
            ecn_list=[self.chemgraph.effective_coordination_number(ha_id) for ha_id in range(self.num_heavy_atoms())]
    ...
    def rep_combine_sort_zero_padd(self, representation_mats, size=9):
        ...
        if self.distances is not None:
            ...
            padded_distances[:self.num_heavy_atoms(), :self.num_heavy_atoms()]=self.distances[inv_canonical_permutation][:, inv_canonical_permutation]
        ...
        sorted_representation_matrices=np.zeros((len(representation_mats), size, size), dtype=float)
        for i, rep_mat in enumerate(representation_mats):
            sorted_representation_matrices[i][:self.num_heavy_atoms(),:self.num_heavy_atoms()]=rep_mat[inv_canonical_permutation][:, inv_canonical_permutation]
        self.representation=np.reshape(sorted_representation_matrices, np.product(sorted_representation_matrices.shape))
    ...
    def add_canon_rdkit_coords(self, canon_rdkit_coords):
        ...
        for hatom_id in range(self.chemgraph.nhatoms()):
            self.coordinates[hatom_id][:]=canon_rdkit_coords[self.additional_data["canon_rdkit_heavy_atom_index"][hatom_id]][:]
        hydrogen_search_lower_bounds=np.zeros(self.chemgraph.nhatoms(), dtype=int)
        for rdkit_hydrogen_id, connected_hatom in self.additional_data["canon_rdkit_hydrogen_connection"].items():
            internal_hydrogen_id=self.find_connected_hydrogen(connected_hatom, start_hydrogen_id=hydrogen_search_lower_bounds[connected_hatom])
            hydrogen_search_lower_bounds[connected_hatom]=internal_hydrogen_id+1
            self.coordinates[internal_hydrogen_id][:]=canon_rdkit_coords[rdkit_hydrogen_id][:]
    ...
    def find_connected_hydrogen(self, hatom_id, start_hydrogen_id=0):
        ...
        for i, (bo, nc) in enumerate(zip(row_adjmat[start_hydrogen_id:], self.nuclear_charges[start_hydrogen_id:])):
            if ((nc==1) and (bo==1)):
