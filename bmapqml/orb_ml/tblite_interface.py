...
def generate_pyscf_mf_mol(oml_compound):
    calculator=tb.Calculator(oml_compound.calc_type, oml_compound.nuclear_charges, oml_compound.coordinates)
    ...
    print("MNKL", res_dict.keys(), res_dict["orbital-energies"])

    pMF=PseudoMF(e_tot=res_dict["energy"], mo_coeff=res_dict["orbital-coefficients"].T, mo_occ=res_dict["orbital-occupations"], mo_energy=res_dict["orbital-energies"])
